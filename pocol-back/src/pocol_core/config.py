...
class Config:
    def __init__(self):
        ...
        self.branches = []
        self.catalogue_file_name = ":MEMORY:"
    ...
    def __dump__(self):
        with open(CONFIG_FILE_NAME, 'w') as f:
            json.dump({"catalogue-file-name": "catalogue.json", "branches": self.branches}, f, sort_keys=True)
    
    def __load__(self):
        with open(CONFIG_FILE_NAME, 'r') as f:
            config = json.load(f)
            if "branches" in config:
                self.catalogue_file_name = config["catalogue-file-name"]
    ...
    def getCatalogueFileName(self):
        return self.catalogue_file_name